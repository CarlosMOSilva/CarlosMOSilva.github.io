...
const generateBoardWithCommonForm = (arr) => {
  ...
  arr.forEach((line, i) => {
    ...
    line.forEach((val, j) => {
      const cell = Cell({
        id: (i * BOARD_SIZE) + j, state: val, row: i, col: j,
      });
      const element = cell.getHTMLElement();
      ...
      updateAliveCells(cell.getState());
      row[j] = cell;
    });
    cells[i] = row;
  });
...
const getNearCellsAlive = (cell) => {
  const isNearCellAlive = (row, col) => cells[row] && cells[row][col]
      && cells[row][col].getState() === 1;

  let cellsAlive = 0;

  if (isNearCellAlive(cell.getRow(), cell.getCol() - 1)) { cellsAlive++; }
  if (isNearCellAlive(cell.getRow(), cell.getCol() + 1)) { cellsAlive++; }
  if (isNearCellAlive(cell.getRow() - 1, cell.getCol() - 1)) { cellsAlive++; }
  if (isNearCellAlive(cell.getRow() - 1, cell.getCol())) { cellsAlive++; }
  if (isNearCellAlive(cell.getRow() - 1, cell.getCol() + 1)) { cellsAlive++; }
  if (isNearCellAlive(cell.getRow() + 1, cell.getCol() - 1)) { cellsAlive++; }
  if (isNearCellAlive(cell.getRow() + 1, cell.getCol())) { cellsAlive++; }
  if (isNearCellAlive(cell.getRow() + 1, cell.getCol() + 1)) { cellsAlive++; }
...
const changeCellsState = () => {
  boardChanged = false;
  const aliveValues = cells.map((line) => line.map((cell) => getNearCellsAlive(cell)));
  cells.forEach((line, i) => line.forEach((cell, j) => {
    if ((cell.getState() === 1 && (aliveValues[i][j] < 2 || aliveValues[i][j] > 3))
        || (cell.getState() === 0 && aliveValues[i][j] === 3)) {
      cell.toggleState();
      boardChanged = true;
    }
  }));
...
btnExport.addEventListener('click', () => {
  alert(JSON.stringify(cells.map((line) => line.map((cell) => cell.getState()))));
});
